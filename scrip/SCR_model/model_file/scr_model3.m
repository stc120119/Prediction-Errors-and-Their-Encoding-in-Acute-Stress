%% Model 3: Noise + Positive Prediction Error (PEp)
clear
...
event_noise_dir = '';
event_PEp_dir = '';
output_dir = fullfile('', 'model3_result');
...
for i = 1:length(data_files)
    ...
    % Load PEp events
    load(event_PEp_file);  % load variable: PEp_events
    ...
    model_timing = struct();
    model_timing.names = {'Noise', 'PEp'};
    model_timing.onsets = {noise_onsets, PEp_onsets};
    model_timing.durations = {0, 0};

    % (Optional) parametric modulation for PEp can be added if needed
